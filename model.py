...
class Trainer:
    ...
    def training(self, train_loader, val_loader, mean, std, method, opt, init_lr = 1e-4, max_epochs = 500, verbose = False):
   
        # transfer learning method:
        if method == 'scratch':
            pass
        elif method == 'finetune':
            self.load(self.model_path)#load source model
        ...
        if opt == 'adam':
            ...
            optimizer = Adam(self.net.parameters(), lr = init_lr, weight_decay = 1e-5)
            ...
            for epoch in range(max_epochs):
                ...
                if verbose:
                    print('--- training epoch %d, lr %f, processed %d, loss %.3f, time elapsed(min) %.2f'
                      %(epoch, optimizer.param_groups[-1]['lr'], train_size, train_loss, (time.time()-start_time)/60))
        
                # validation
                start_time = time.time()
                val_y_preds = self.inference(val_loader, mean, std) 
    
                val_loss = mean_squared_error(val_y, val_y_preds) ** 0.5
                lr_scheduler.step(val_loss)
                val_log[epoch] = val_loss
        
                if verbose:
                    print('--- validation at epoch %d, total processed %d, RMSE %.3f, BEST %.3f, monitor %d, time elapsed(min) %.2f'
                      %(epoch, val_size, val_loss, np.min(val_log[:epoch + 1]), epoch - np.argmin(val_log[:epoch + 1]), (time.time()-start_time)/60))  
            ...
            for epoch in range(max_epochs):
                ...
                # save minimin-loss parameters
                if np.argmin(val_log[:epoch + 1]) == epoch:
                    torch.save(self.net.state_dict(), self.target_path) 
    
            print('training terminated at iter %d'%np.argmin(val_log))
            self.load(self.target_path)
